...
| Security/isolation static gate | TESTED IN CI | Checked-in public binds, floating versions, credential/private-key literals and unsafe host-network/privileged defaults are rejected |
...
| Host evidence collector | IMPLEMENTED | Read-only service/network/config evidence capture for Gate 4E/5/7; gracefully records unavailable systemd tools instead of emitting command-not-found noise |
...
`scripts/project-72-backup.sh` backs up canonical subscriber, durable idempotency and optional eSIM metadata databases independently, with restricted permissions and checksums. Raw secret material and private keys are excluded. The backup uses MongoDB Database Tools `--config` for sensitive connection URIs so credentials are not placed in the `mongodump` process arguments, and rejects empty archives. `docs/backup-recovery.md` defines isolated restore validation, index/schema checks and the fail-closed rule for uncertain telecom side effects.
...
The current test environment is an Ubuntu 22.04 userland reached through `proot-distro` on an ARM64 Android host. The evidence refresh shows no `systemctl`, no `ogstun`, unavailable kernel forwarding control and unavailable iptables policy access. This is therefore not a compliant runtime host. The runtime preflight correctly blocks provisioning rather than weakening the service/network checks. It remains useful for source builds and deterministic validation.
