...
def _sample(name: str, value: int, labels: dict[str, str] | None = None) -> str:
    label_text = ""
    if labels:
...
def render(snapshot: dict[str, Any]) -> str:
    lines = ["# HELP mm7_assurance_operations_total Project-72 assurance operations by terminal status.", "# TYPE mm7_assurance_operations_total counter"]
    ...
    for status in ASSURANCE_STATUSES:
        ...
        lines.append(_sample("mm7_assurance_operations_total", value, {"status": status}))
    ...
    lines += ["# HELP mm7_drift_events_total Project-72 authoritative drift classifications.", "# TYPE mm7_drift_events_total counter", _sample("mm7_drift_events_total", drift)]

    subscribers = snapshot.get("subscribers", {})
    if not isinstance(subscribers, dict):
        raise ValueError("subscribers must be an object")
    lines += ["# HELP mm7_subscriber_health Subscriber health from the latest authoritative evidence snapshot.", "# TYPE mm7_subscriber_health gauge"]
    for subscriber_id in SUBSCRIBERS:
        ...
        lines.append(_sample("mm7_subscriber_health", int(healthy), {"subscriber_id": subscriber_id}))
    return "\n".join(lines) + "\n"
