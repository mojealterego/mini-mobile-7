...
class EsimArtifactGenerator:
    """Build an LPA activation artifact from externally supplied SM-DP+ data.

    This class does not manufacture an eSIM profile or contact an SM-DP+.
    The activation token/matching ID must come from the real provisioning
    system. Consequently a generated artifact is not equivalent to an
    installed or verified eSIM.
    """
    ...
    def _validate_metadata(metadata: EsimProfileMetadata) -> None:
        ...
        if not metadata.smdp_address or any(ch.isspace() for ch in metadata.smdp_address):
            raise EsimProvisioningError("smdp_address must be a non-empty host-like value")
