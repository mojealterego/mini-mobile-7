...
class RuntimeProvisionerWiringTest(TestCase):
    ...
    def test_runtime_execute_requires_preflight(self) -> None:
        source = self._source()
        self.assertIn("def run_preflight(mongodb_uri: str) -> bool:", source)
        ...
        self.assertIn("if not run_preflight(mongodb_uri):", source)
        self.assertIn("no provisioning mutation permitted", source)

    def test_runtime_preflight_uses_same_mongodb_uri_as_execution(self) -> None:
        source = self._source()
